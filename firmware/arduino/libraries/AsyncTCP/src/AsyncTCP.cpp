...
class TcpipCoreLockGuard {
 public:
  TcpipCoreLockGuard() : locked_(false) {
...
static inline void _tcp_arg_with_lock(tcp_pcb *pcb, void *arg) {
    if(!pcb){
        return;
    }
#if defined(CONFIG_LWIP_TCPIP_CORE_LOCKING) && CONFIG_LWIP_TCPIP_CORE_LOCKING
    TcpipCoreLockGuard guard;
#endif
    tcp_arg(pcb, arg);
}

static inline void _tcp_recv_with_lock(tcp_pcb *pcb, tcp_recv_fn fn) {
    if(!pcb){
        return;
    }
#if defined(CONFIG_LWIP_TCPIP_CORE_LOCKING) && CONFIG_LWIP_TCPIP_CORE_LOCKING
...
static inline void _tcp_sent_with_lock(tcp_pcb *pcb, tcp_sent_fn fn) {
    if(!pcb){
        return;
    }
#if defined(CONFIG_LWIP_TCPIP_CORE_LOCKING) && CONFIG_LWIP_TCPIP_CORE_LOCKING
...
static inline void _tcp_err_with_lock(tcp_pcb *pcb, tcp_err_fn fn) {
    if(!pcb){
        return;
    }
#if defined(CONFIG_LWIP_TCPIP_CORE_LOCKING) && CONFIG_LWIP_TCPIP_CORE_LOCKING
...
static inline void _tcp_poll_with_lock(tcp_pcb *pcb, tcp_poll_fn fn, u8_t interval) {
    if(!pcb){
        return;
    }
#if defined(CONFIG_LWIP_TCPIP_CORE_LOCKING) && CONFIG_LWIP_TCPIP_CORE_LOCKING
    TcpipCoreLockGuard guard;
#endif
    tcp_poll(pcb, fn, interval);
}

static inline void _tcp_accept_with_lock(tcp_pcb *pcb, tcp_accept_fn fn) {
    if(!pcb){
        return;
    }
#if defined(CONFIG_LWIP_TCPIP_CORE_LOCKING) && CONFIG_LWIP_TCPIP_CORE_LOCKING
...
static err_t _tcp_new_api(struct tcpip_api_call_data *api_call_msg){
    tcp_new_api_call_t * msg = (tcp_new_api_call_t *)api_call_msg;
    msg->pcb = tcp_new_ip_type(msg->type);
    return msg->pcb ? ERR_OK : ERR_MEM;
}

static tcp_pcb * _tcp_new_ip_type_safe(uint8_t type){
#if defined(CONFIG_LWIP_TCPIP_CORE_LOCKING) && CONFIG_LWIP_TCPIP_CORE_LOCKING
    if (sys_thread_tcpip(LWIP_CORE_LOCK_QUERY_HOLDER)) {
        return tcp_new_ip_type(type);
    }
#endif
    tcp_new_api_call_t msg = {};
    msg.type = type;
...
, next(NULL)
{
    ...
    if(_pcb){
        ...
        _tcp_arg_with_lock(_pcb, this);
        _tcp_recv_with_lock(_pcb, &_tcp_recv);
        _tcp_sent_with_lock(_pcb, &_tcp_sent);
        _tcp_err_with_lock(_pcb, &_tcp_error);
        _tcp_poll_with_lock(_pcb, &_tcp_poll, 1);
    }
}
...
bool AsyncClient::connect(IPAddress ip, uint16_t port){
    ...
    tcp_pcb* pcb = _tcp_new_ip_type_safe(IPADDR_TYPE_V4);
    if (!pcb){
        log_e("pcb == NULL");
        return false;
    }

    _tcp_arg_with_lock(pcb, this);
    _tcp_err_with_lock(pcb, &_tcp_error);
    _tcp_recv_with_lock(pcb, &_tcp_recv);
    _tcp_sent_with_lock(pcb, &_tcp_sent);
    _tcp_poll_with_lock(pcb, &_tcp_poll, 1);
...
int8_t AsyncClient::_close(){
    ...
    if(_pcb) {
        //log_i("");
        _tcp_arg_with_lock(_pcb, NULL);
        _tcp_sent_with_lock(_pcb, NULL);
        _tcp_recv_with_lock(_pcb, NULL);
        _tcp_err_with_lock(_pcb, NULL);
        _tcp_poll_with_lock(_pcb, NULL, 0);
...
void AsyncServer::begin(){
    ...
    int8_t err;
    _pcb = _tcp_new_ip_type_safe(IPADDR_TYPE_V4);
    if (!_pcb){
        log_e("_pcb == NULL");
        return;
    }
    ...
    _tcp_arg_with_lock(_pcb, (void*) this);
    _tcp_accept_with_lock(_pcb, &_s_accept);
}

void AsyncServer::end(){
    if(_pcb){
        _tcp_arg_with_lock(_pcb, NULL);
        _tcp_accept_with_lock(_pcb, NULL);
        if(_tcp_close(_pcb, -1) != ERR_OK){
            _tcp_abort(_pcb, -1);
        }
        _pcb = NULL;
    }
